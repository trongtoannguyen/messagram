...
> **Last consolidated:** 2026-09-07 (added #16 — locked JDK 21 / Spring Boot 4.1.x / Gradle 9.x for Implementation)   
> **Authority:** This document records product and architecture decisions specific to this project. `AGENTS.md` defines
> the operating principles and process. If they appear to conflict, this document governs project-specific decisions; the
...
| #16 | 2026-09-07 | Active            | Lock concrete versions for Implementation: **JDK 21 (LTS)**, **Spring Boot 4.1.x**, **Gradle 9.x** (Kotlin DSL). Dependency versions for individual libraries (Jackson, Hibernate, SLF4J, etc.) are **not** pinned manually — they are resolved via Spring Boot's dependency-management BOM based on the declared Spring Boot version.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | JDK 21 is LTS (support to ~2029+), already installed on the founder's machine, and includes stable (non-preview) virtual threads — directly relevant to Realtime Service's many-concurrent-WebSocket-connection workload. Spring Boot 3.x is required for JDK 21 support (2.x does not fully support JDK 21); Gradle 8.x is required to run on JDK 21. JDK 25 was considered and rejected: not LTS, short support window, higher risk of plugin/library incompatibility for a new project — not justified without a concrete need. Manual per-library version pinning was rejected as unnecessary complexity; Spring Boot's BOM already solves cross-library version compatibility.                                                         | Revisit if a specific library requires a JDK/Spring Boot version incompatible with this choice, or when the next Spring Boot LTS-aligned minor version supersedes 3.3.x with a concrete reason to move.                                                                                                                                                                                                                                |
...
> API/WebSocket contracts are deliberately out of scope for this section — they belong to the next lifecycle step (Section
8) and must be derived from this model, not defined ahead of it.
